  ...
  public function storeMobileNumber(Request $request)
{
    ...
    $request->validate([
        'mobile_number' => 'required|numeric',  // You can adjust the validation as needed
    ]);
  ...
  public function registeredBusinessPhoneNumbers(Request $request)
{
    ...
    // Create a new entry in the UsersPhoneNumbers table
    $userMobileNo = new \App\Models\RegisteredBusinessPhoneNumbers();  // Create a new model instance
    $userMobileNo->user_registered_business_id = $userId;  // Set the user_id
    $userMobileNo->phone_number = $request->mobile_number;  // Set the mobile number
    $userMobileNo->save();  // Save the record
    ...
    public function seller_card_info(Request $request)
    {
        // Validate the input
        // $request->validate([
        //     'seller.first_name' => 'nullable|string|max:255',
        //     'seller.middle_name' => 'nullable|string|in:state_owned,publicly_listed,privately_owned,individual',
        //     'seller.last_name' => 'nullable|string|max:255',
        //     'seller.country_of_citizenship' => 'nullable|string|max:255',
        //     'seller.date_of_birth_day' => 'nullable|string|max:255',
        //     'seller.date_of_birth_month' => 'nullable|string|max:20',
        //     'seller.date_of_birth_year' => 'nullable|string|max:255',
        //     'seller.Registered_business_address' => 'nullable|string|max:255',
        //     'seller.zip_postal_code' => 'nullable|string|max:255',
        //     'seller.address_line1' => 'nullable|string|max:255',
        //     'seller.appartment_building_suite_other' => 'nullable|string|max:255',
        //     'seller.city_town' => 'nullable|string|max:255',
        //     'seller.state_region' => 'nullable|string|max:255',
        //     'seller.beneficial_owner_check' => 'nullable|string|max:255',
        //     'seller.legal_representative_check' => 'nullable|string|max:255',
        //     'seller.primary_contact_person_check' => 'nullable|string|max:255',
        // ], [
        //     'seller.first_name.max' => 'The first name must not exceed 255 characters.',
        //     'seller.middle_name.in' => 'Please select a valid middle name option.',
        //     'seller.last_name.max' => 'The last name must not exceed 255 characters.',
        //     'seller.country_of_citizenship.max' => 'The country of citizenship must not exceed 255 characters.',
        //     'seller.date_of_birth_day.max' => 'The date of birth day must not exceed 255 characters.',
        //     'seller.date_of_birth_month.max' => 'The date of birth month must not exceed 20 characters.',
        //     'seller.date_of_birth_year.max' => 'The date of birth year must not exceed 255 characters.',
        //     'seller.Registered_business_address.max' => 'The registered business address must not exceed 255 characters.',
        //     'seller.zip_postal_code.max' => 'The zip/postal code must not exceed 255 characters.',
        //     'seller.address_line1.max' => 'The address line 1 must not exceed 255 characters.',
        //     'seller.appartment_building_suite_other.max' => 'The apartment/building/suite/other field must not exceed 255 characters.',
        //     'seller.city_town.max' => 'The city/town must not exceed 255 characters.',
        //     'seller.state_region.max' => 'The state/region must not exceed 255 characters.',
        //     'seller.beneficial_owner_check.max' => 'The beneficial owner check field must not exceed 255 characters.',
        //     'seller.legal_representative_check.max' => 'The legal representative check field must not exceed 255 characters.',
        //     'seller.primary_contact_person_check.max' => 'The primary contact person check field must not exceed 255 characters.',
        // ]);
        ...
        try {
            ...
            if ($user) {
                ...
                $user->update([
                    'first_name' => $sellerInfo['first_name'] ?? $user->first_name,
                    'middle_name' => $sellerInfo['middle_name'] ?? $user->middle_name,
                    'last_name' => $sellerInfo['last_name'] ?? $user->last_name,
                ...
                return redirect()->route('seller.login')->with('success', 'User details updated successfully.');
            } else {
    ...
    public function showPreviousPage()
{
    ...
    public function users_registered_business(Request $request)
    {
        ...
        try {
            ...
            if ($business) {
                ...
                UsersRegisteredBusiness::create([
    ...
    //         return redirect()->back(); // Ensures proper redirection on expiry
    //     }
    //     // if (now()->diffInSeconds(session('otp_time')) > 10) {
    //     //     Toastr::error(__('otp expired'), __('common.error'));
    //     //     return view(theme('pages.otp')); // Ensures proper redirection on expiry
    //     // }
    ...
    //         // Toastr::success(__('verified successfully') . ' ' . __('auth.please_verify_your_email'), __('common.success'));
    
    //         // Redirect to the desired view
    //         return view(theme('pages.sellergetdata'));// Replace with the correct route name
    //     } else {
    //         // dd('error');
    //         // OTP does not match
    //         Toastr::error(__('invalid otp'), __('common.error'));
    //         return view(theme('pages.otp'));// Redirect back to the same page with an error
    //         // return view(theme('pages.otp'));// Redirect back to the same page with an error
    //         // return view(theme('pages.sellergetdata'));// Replace with the correct route name
    //     }
    // }
