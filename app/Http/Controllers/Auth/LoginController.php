...
class LoginController extends Controller
{
    ...
    public function showLoginForm()
    {
        ...
        if (url()->previous() == url('/cart')) {
            session()->put('from_cart', url('/cart')); // Store the cart URL in session
        }
        // if(url()->previous() == url('/checkout') || url()->previous() == url('/checkout?checkout_type=YnV5X2l0X25vdw==')){
        //     session()->put('from_checkout',url('/'));
        // }
    ...
    public function login(Request $request)
    {
        ...
        if (!$check_user) {
            $check_user = User::where('username', $request->login)
                              ->where('is_active', 1)
                              ->whereHas('role', function($q) {
                                  return $q->where('type', 'customer');
                              })->first();
        ...
        if ($check_user) {
            ...
            return redirect()->intended('/profile/dashboard'); // This will redirect to dashboard or intended URL
        } else {
            throw ValidationException::withMessages([
                "email" => __('auth.failed')
            ]);
