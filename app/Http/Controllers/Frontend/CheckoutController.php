...
class CheckoutController extends Controller
{
    ...
    public function __construct(CheckoutService $checkoutService, PaymentGatewayService $paymentGatewayService, SpeedShipService $speedShipService)
    {
    ...
    {
        $sub_discount = $request->input('sub_discount');
        // dd($sub_discount);
        ...
        if ($step == 'select_shipping') {
            ...
            // $infoCompleteOrder['sub_discount'] = $sub_discount;
            session()->put('infoCompleteOrder', $infoCompleteOrder);
            $shippingData = session()->get('package_wise_shipping');
            // dd($shippingData);
            $SpeedshipAuthToken = SpeedshipAuthToken::first();
            $shippingAddress = auth()->user()->customerAddresses->first()
                ?? CustomerAddress::where('customer_id', auth()->user()->id)->first();
            // Extract values with null coalescing and defaults
            $country = $shippingAddress->getCountry->code ?? "US";
            $state = $shippingAddress->getState->name ?? "N/A";
            $city = $shippingAddress->getCity->name ?? "N/A";
            $addressLine = $shippingAddress->address ?? "N/A";
            $postalCode = $shippingAddress->postal_code ?? "N/A";
            $phone = $shippingAddress->phone ?? "N/A";
            // dd($country,$state,$city,$addressLine,$postalCode,$phone);
            $package_type = $SpeedshipAuthToken->package_type;
            // Split the string into two parts: code and name
            list($packagingType, $packagingTypeName) = explode(',', $package_type);
            // dd($shipping_address,$customer,$countries,$states,$cities);
            // dd($shipping_address);
            ...
            $domesticShopFlowPayload = [
                "request" => [
                    ...
                    "shipment" => [
                        "shipmentDate" => (string) Carbon::now()->addDays(3)->format('Y-m-d H:i:s'),
                        "originAddress" => [
                            "address" => [
                                "addressLineList" => [
                                    (string) app('general_setting')->address ?? "N/A",
                                ],
                                "locality" => (string) app('general_setting')->getCity->name ?? "N/A",
                                "region" => (string) app('general_setting')->getState->name ?? "N/A",
                                "postalCode" => (string) app('general_setting')->zip_code ?? "N/A",
                                "countryCode" => (string) app('general_setting')->getCountry->code ?? "N/A",
                                "companyName" => (string) app('general_setting')->company_name ?? "N/A",
                                "phone" => (string) app('general_setting')->phone ?? "N/A",
                                "contactList" => [
                                    [
                                        "firstName" => (string) app('general_setting')->site_title ?? "N/A",
                                        "lastName" => (string) app('general_setting')->site_title ?? "N/A",
                                        "phone" => (string) app('general_setting')->phone ?? "N/A",
                                        "contactType" => "SENDER",
                                        "fax" => null,
                                        "website" => null
                        ...
                        "destinationAddress" => [
                            "address" => [
                                ...
                                "contactList" => [
                                    [
                                        "firstName" => (string) auth()->user()->first_name ?? "N/A",
                                        "lastName" => (string) auth()->user()->last_name ?? "N/A",
                                        "phone" => (string) $phone,
                        ...
                        "handlingCharge" => [
                            "value" => $SpeedshipAuthToken->handling_charges ?? 0,
                            "unit" => "PERCENT"
                        ],
                        "handlingUnitList" => [
                            [
                                "billedDimension" => [
                                    "length" => [
                                        "value" => (string) number_format($shippingData['totalItemLength'] / 2.54, 2, '.', ''),
                                        "unit" => "in"
                                    ],
                                    "width" => [
                                        "value" => (string) number_format($shippingData['totalItemBreadth'] / 2.54, 2, '.', ''),
                                        "unit" => "in"
                                    ],
                                    "height" => [
                                        "value" => (string) number_format($shippingData['totalItemHeight'] / 2.54, 2, '.', ''),
                                        "unit" => "in"
                                    ]
                                ],
                                "packagingType" => (string) $packagingType ?? "02",
                                "packagingTypeName" => (string) $packagingTypeName ?? "Custom",
                                "quantity" => 1,
                                // "referenceList" => [
                                //     [
                                //         "type" => "Reference 1",
                                ...
                                // ],
                                "weight" => [
                                    "value" =>  (string) number_format($shippingData['totalItemWeight'] / 453.592, 2, '.', ''),
                                    "unit" => "LB"
                                ]
                            ]
                        ],
                        ...
                        "totalWeight" => [
                            "value" => number_format($shippingData['totalItemWeight'] / 453.592, 2, '.', ''),
                            "unit" => "LB"
                        ],
            ...
            // dd($domesticShopFlowPayload);
            $shippingCharges = $this->speedShipService->shopFlowDomestic($domesticShopFlowPayload);
            // dd($shippingCharges['response']);
            // dd($shippingCharges);
            $newShippingCharge = $request->new_shipping_method;
            return view(theme('pages.payment_step'), array_merge($infoCompleteOrder, ['sub_discount' => $sub_discount, 'newShippingCharge' => $newShippingCharge, 'shippingCharges' => $shippingCharges]));
        }
        if ($step == 'complete_order') {
            ...
            $shippingCharge = $request->get('shipping_charge', 0);
            $subDiscount = $request->get('sub_discount', 0);
            $subDiscount = ltrim($subDiscount, '-');
            // new flow
            $orderData = [
        ...
        // dd("shipping_address",$shipping_address,"shipping_cost",$shipping_cost,"pickup_locations",$pickup_locations,"free_shipping_for_pickup_location",$free_shipping_for_pickup_location);

        return view(theme('pages.checkout'), compact(
            'shipping_methods',
            'cartData',
            'shipping_address',
            'gateway_activations',
    ...
    public function changeShippingMethod(Request $request)
    {
        ...
        $checkoutField = CheckoutFieldVisibility::all();
        return view(theme('partials._checkout_details'), compact(
            'shipping_methods',
            'cartData',
            'shipping_address',
            'gateway_activations',
