...
use Modules\MultiVendor\Entities\Commission;
...
class OrderRepository
{
    ...
    {
        // dd($order);
        // Ensure $order is loaded with necessary relationships
        $order->load([
            'packages.products.seller_product_sku.seller',
        ]);
        // Initialize a collection to group products by seller
        $sellerGroups = [];
    
        foreach ($order->packages as $package) {
            foreach ($package->products as $product) {
                $sku = $product->seller_product_sku;
                $seller = $sku?->seller;
    
                if (!$seller) {
                    continue; // Skip if no seller is associated
                }
    
                // Group products by seller
                $sellerGroups[$seller->id][] = [
                    'product_id' => $product->id,
                    'price' => $product->price, // Ensure price is available in OrderProductDetail
                    'qty' => $product->qty, // Ensure quantity is available
                ];
            }
        }
        // Calculate commission for each seller
        foreach ($sellerGroups as $sellerId => $products) {
            $totalSaleAmount = 0;
    
            foreach ($products as $product) {
                $totalSaleAmount += $product['price'] * $product['qty'];
            }

            $sellerCommissionRate = SellerCommssionType::where([
                ['name', '=', 'Commission'],
                ['status', '=', 1],
            ])->value('rate');
            // dd($sellerCommissionRate);
            $commissionAmount = ($totalSaleAmount * $sellerCommissionRate) / 100;
    ...
    {
        // dd($amount, $method, $response, $user = null);
