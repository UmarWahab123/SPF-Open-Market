...
class ProductController extends Controller
{
    ...
    public function create(CategoryService $categoryService, UnitTypeService $unitTypeService, BrandService $brandService, TagService $tagService, AttributeService $attributeService, ShippingService $shippingService,GSTService $gstService){
        ...
        $business_info = UsersRegisteredBusiness::where('user_id', $seller_id)->first();
        // $business_info = UsersRegisteredBusiness::with('country', 'state', 'city')->where('user_id', $seller_id)->first();
        // $business_info = UsersRegisteredBusiness::where('user_id', session('user_id'))->first();
        $bankInfo = $this->productService->getSellerBankInfo();
        // dd($bankInfo);
        if($business_info ==null || $business_info->business_name == null || $business_info->address_line1 == null
        || $business_info->address_line1 == null || $business_info->country == null
        || $business_info->state_region == null || $business_info->city_town == null){
    ...
    public function store(Request $request){
        ...
        try{
            ...
            $seller_id = getParentSellerId();
            $business_info = UsersRegisteredBusiness::where('user_id', $seller_id)->first();
            $bankInfo  = UserPaymentsInfo::where('user_id', $seller_id)->first();
