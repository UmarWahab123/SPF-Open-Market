...
@section('mainContent')
        ...
        <div class="{{$activeAutomaticCarrier > 0 ? 'col-md-5 col-sm-6 col-xs-12': "col-md-12 col-sm-12 col-xs-12"}}">
            <div class="main-title mb-25 d-md-flex">
                ...
                {{-- <ul class="d-flex d-none">
                    @if(permissionCheck('shipping.carriers.store'))
                        <li>
                            <a  data-toggle="modal" data-target="#access_token_add_carrier_modal" class="primary-btn radius_30px mr-10 fix-gr-bg" href="#">
            ...
            <div class="common_QA_section QA_section_heading_custom">
                <div class="QA_table ">
                    <!-- table-responsive -->
                    <div id="carrier_list">
                        @include('shipping::carriers.new_list')
                    </div>
                </div>
            </div>
        </div>
    ...
    @include('shipping::carriers.token_api_create')
    ...
    $(document).ready(function(){
        ...
        $(document).on('submit', '#create_form', function(event){
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    create_form_reset();
                    $('#access_token_add_carrier_modal').modal('hide');
        ...
        function showValidationErrors(formType, errors) {
            @if(isModuleActive('FrontendMultiLang'))
                // Name field validation for multi-language
            ...
            // Additional form fields validation
            $(formType + ' #error_handling_charges').text(errors.handling_charges);
            $(formType + ' #error_client_id').text(errors.client_id);
            $(formType + ' #error_client_secret').text(errors.client_secret);
            $(formType + ' #error_audience').text(errors.audience);
            $(formType + ' #error_auth_url').text(errors.auth_url);
            $(formType + ' #error_url').text(errors.url);
             $(formType + ' #error_package_type').text(errors.package_type);
        }
        ...
        function resetValidationError() {
            ...
            // Reset additional form fields validation errors
            $('#error_handling_charges').text('');
            $('#error_client_id').text('');
            $('#error_client_secret').text('');
            $('#error_audience').text('');
            $('#error_auth_url').text('');
            $('#error_url').text('');
           $('#error_package_type').text(''); 
        }
    });
})(jQuery);
</script>
