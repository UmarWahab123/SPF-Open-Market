...
Route::middleware(['admin','auth'])->prefix('admin')->as('admin.')->group(function(){
    ...
    Route::resource('/subscription-plan', 'PricingPlanController')->except('destroy, update');
    Route::post('/subscription-plan/delete','PricingPlanController@destroy')->name('subscription-plan.delete')->middleware('permission');
    Route::post('/subscription-plan/update','PricingPlanController@update')->name('subscription-plan.update')->middleware('permission');
    Route::post('/subscription-plan/status-update','PricingPlanController@status')->name('subscription-plan.status')->middleware('permission');
       // discount role 
    Route::resource('/discount-role', 'DiscountRoleController')->except('destroy, update,index');
    Route::get('/discount-role/index/{id?}','DiscountRoleController@index')->name('discount.role.index')->middleware('permission');
    Route::post('/discount-role/delete','DiscountRoleController@destroy')->name('discount.role.delete')->middleware('permission');
    Route::post('/discount-role/update','DiscountRoleController@update')->name('discount.role.update')->middleware('permission');
    Route::post('/discount-role/status-update','DiscountRoleController@status')->name('discount.role.status')->middleware('permission');
});
