...
class MerchantController extends Controller
{
    ...
    public function getData(Request $request){
        ...
        return DataTables::of($seller)
            ...
            ->addColumn('Phone_number', function($seller){
                return getNumberTranslate(@$seller->user->Phone_number ?? 'X');
            })
            ->addColumn('shop_name', function($seller){
                return optional($seller->user->SellerRegisteredBusiness)->business_name ?? 'X';
            })
            ->addColumn('total_orders', function($seller){
                return getNumberTranslate(count(@$seller->user->order_packages));
            })
            ->addColumn('is_active', function($seller) {
                $checked = $seller->user->is_active == 1 ? 'checked' : '';            
                return '
                    <span class="ml-1"></span>
                    <label class="switch_toggle" for="active_checkbox' . $seller ->user->id . '">
                        <input type="checkbox" id="active_checkbox' . $seller->user->id . '" ' . $checked . ' value="' . $seller->user->id . '" class="seller-is-active-status-update" data-id="' . $seller->user->id . '" data-seller-id="' . $seller->user->id . '">
    ...
    public function is_active_status_update(Request $request)
    {
        try {
            $seller = User::find($request->seller_id);
        
            if ($seller) {
                $seller->is_active = $request->is_active ? 1 : 0; // Simplified condition
                $seller->save();
        
                LogActivity::successLog('Seller active status successfully updated.');
                return 1;
            }
            LogActivity::errorLog('Seller not found.');
            return 0;
        } catch (\Exception $e) {
    ...
    //below method is for the seller subscriptio payment through paypal 
    
    // public function is_active_status_update(Request $request)
    // {
    //     try {
    //         // Step 1: Fetch the seller's payment info
    //         $seller_payment_info = UserPaymentsInfo::where('user_id', $request->seller_id)->first();
            
    //         if (!$seller_payment_info) {
    //             return response()->json(['error' => 'Payment information not found for this seller.'], 404);
    //         }
    
    //         // Step 2: Get PayPal credentials
    //         $clientId = config('services.paypal.client_id'); // PayPal Client ID
    //         $clientSecret = config('services.paypal.secret'); // PayPal Client Secret
    ...
    //             'log.LogLevel' => 'DEBUG', // Use 'DEBUG' for detailed logs
    //         ]);
    
    //         // Step 3: Extract card details
    //         $cardNumber = $seller_payment_info->card_number;
    //         $expiry = explode('-', $seller_payment_info->expire_in);
    //         $expireMonth = $expiry[0]; // Month
    //         $expireYear = $expiry[1]; // Year
    //         $cardHolderName = $seller_payment_info->card_holder_name;
    
    //         // Step 4: Create a Credit Card Object
    //         $card = new CreditCard();
    //         $card->setType("visa") // Adjust based on card type
    //             ->setNumber($cardNumber)
    //             ->setExpireMonth($expireMonth)
    //             ->setExpireYear($expireYear)
    //             ->setCvv2("123") // Replace with actual CVV if available
    //             ->setFirstName($cardHolderName)
    //             ->setLastName("Dev"); // Adjust based on cardholder's actual last name
    
    //         // Step 5: Create Funding Instrument
    //         $fundingInstrument = new FundingInstrument();
    //         $fundingInstrument->setCreditCard($card);
    
    //         // Step 6: Create a Payer
    //         $payer = new Payer();
    //         $payer->setPaymentMethod("credit_card")
    //             ->setFundingInstruments([$fundingInstrument]);
    
    //         // Step 7: Define Transaction Amount
    //         $amount = new Amount();
    //         $amount->setCurrency("USD")
    //             ->setTotal("39.00"); // Adjust to your actual amount
    
    //         // Step 8: Specify Payee
    //         $payee = new Payee();
    //         $payee->setEmail("sb-2b07h33728984@business.example.com"); // Replace with seller's PayPal email
    //         // dd($payee);
    //         // Step 9: Create a Transaction
    //         $transaction = new Transaction();
    //         $transaction->setAmount($amount)
    //             ->setDescription("Transaction for seller subscription update")
    //             ->setPayee($payee); // Set payee information
    ...
    //         $payment->create($apiContext); // Execute the payment on PayPal
    ...
    //         return response()->json([
    //             'error' => json_decode($ex->getData()) // Return more details about the PayPal error
    //         ]);
    //     } catch (\Exception $e) {
    //         // Handle other errors
    //         return response()->json([
    ...
    {
        $isDeleted = $this->merchantService->deleteSellerByID($id);
    
        if ($isDeleted) {
            Toastr::success(__('Seller deleted successfully'),__('common.success'));
            return redirect()->back();
        } else {
            Toastr::error(__('common.operation_failed'));
            return back();
