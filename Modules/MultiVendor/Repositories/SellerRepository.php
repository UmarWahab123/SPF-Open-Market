...
class SellerRepository {
    ...
    public function orderCommissionForAdminViaFilter($data = 'today') {
        ...
        if ($data == 'today') {
            $payments = OrderPayment::whereBetween('created_at', [Carbon::now()->format('Y-m-d')." 00:00:00", Carbon::now()->format('Y-m-d')." 23:59:59"])
                ->where('amount_goes_to_seller', 1)
                ->where('commision_amount', '>', 0)
                ->whereHas('order', function($query) {
                    $query->whereHas('packages', function($q) {
                        $q->where('seller_id', getParentSellerId());
                    });
                });
        } elseif ($data == 'week') {
            $payments = OrderPayment::whereBetween('created_at', [Carbon::now()->subDays(7)->format('Y-m-d')." 00:00:00", Carbon::now()->format('Y-m-d')." 23:59:59"])
                ->where('amount_goes_to_seller', 1)
                ->where('commision_amount', '>', 0)
                ->whereHas('order', function($query) {
                    $query->whereHas('packages', function($q) {
            ...
            $month = Carbon::now()->month;
            $date_1 = Carbon::create($year, $month)->startOfMonth()->format('Y-m-d')." 00:00:00";
            $payments = OrderPayment::whereBetween('created_at', [$date_1, Carbon::now()->format('Y-m-d')." 23:59:59"])
                ->where('amount_goes_to_seller', 1)
                ->where('commision_amount', '>', 0)
                ->whereHas('order', function($query) {
                    $query->whereHas('packages', function($q) {
