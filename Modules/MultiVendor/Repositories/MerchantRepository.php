...
class MerchantRepository
{
    ...
    {
        return SellerAccount::with('user')->whereHas('user', function($query){
            $query->with('SellerRegisteredBusiness', 'SellerPaymentsInfo', 'SellerPhoneNumbers')
            ->where('is_active', 1)
            ->where('id', '>', 1);
        });
    }
    ...
    {
        return User::with('SellerRegisteredBusiness', 'SellerPaymentsInfo', 'SellerPhoneNumbers', 'order_packages', 'seller_products')->findOrFail($id);
