...
<nav id="sidebar" class="sidebar">
    <div class="sidebar-header update_sidebar">
        <a class="large_logo" href="{{ auth()->user()->role->type == 'seller' ? route('seller.dashboard') : route('admin.dashboard') }}">
            <img src="{{asset("public/uploads/images/logo-spf-08.png")}}"> 
    ...
    !(auth()->user()->role->type == 'seller' && $section->backendMenu->id == 132) && 
    (auth()->user()->role->type != 'seller' || !in_array($section->backendMenu->id, [132])))
                    ...
                    @foreach($section->children as $menu)
                        ...
                        @if(!@$menu->backendMenu->module or isModuleActive(@$menu->backendMenu->module))
                            @if(@$menu->backendMenu->route == 'payment_gateway.index' && auth()->user()->role->type == 'seller' && !app('general_setting')->seller_wise_payment)
                                @continue
                           @elseif(permissionCheck(@$menu->backendMenu->route) && !(auth()->user()->role->type == 'seller' && $menu->backendMenu->id == 67) && !in_array($menu->backendMenu->id, [179, 150]))
                                        ...
                                        <ul class="mm-collapse">
                                            @foreach($menu->children as $submenu)
                                                ...
                                                @if(!@$submenu->backendMenu->module or isModuleActive(@$submenu->backendMenu->module))
                                                    @if(permissionCheck($submenu->backendMenu->route) && !in_array($submenu->backendMenu->id, [202,203,208]))
                                                        <li>
                                                            <a href="
