...
<?php } ?>
    <script>
        ...
        $(document).ready(function () {
            const originalSubtotal = parseFloat($("#total_amount").data("amount"));
            let paypalButtonRendered = false;

            // Function to re-render PayPal button
            function renderPayPalButton(totalAmount) {
                $("#paypal-button-container").html(''); // Clear the old button
                paypal.Buttons({
                    ...
                    createOrder: function (data, actions) {
                        ...
                        return actions.order.create({
                            purchase_units: [{
                                amount: {
                                    value: totalAmount // Use the updated total amount
                                }
                            }]
                        });
                    },
                            ...
                            .then(data => {
                                ...
                                // Redirect based on the payment type
                                    if (data.type === 'order') {
                                        ...
                                        const shippingCharge = parseFloat($("#shipping_charge span").text().replace(/[^\d.-]/g, '')) || 0; // Remove any non-numeric characters
                                        const subscriptionDiscount = parseFloat($("#subscribtion_discount span").text().replace(/[^\d.-]/g, '')) || 0;
                                        let redirectData = {
                                       ...
                                       toastr.error("{{__('Something is wrong')}}","{{__('common.error')}}");
                ...
                }).render('#paypal-button-container'); // Re-render the PayPal button
            }

            // Shipping method change handler
            $(".shipping_method_radio").on("change", function () {
                const selectedShippingCharge = parseFloat($(this).val());
                // Update the shipping charge display
                $("#shipping_charge").html(`<span>+ $${selectedShippingCharge.toFixed(2)}</span>`);
                const newTotal = (originalSubtotal + selectedShippingCharge).toFixed(2);
                ...
                // Re-render the PayPal button with updated total
                renderPayPalButton(newTotal);
            });

            // Initial render of PayPal button
            const initialTotal = $("#total_amount").data("amount");
            renderPayPalButton(initialTotal);
        });

    //    $(document).ready(function () {
    //         // Store the original subtotal for reference
    //         const originalSubtotal = parseFloat($("#total_amount").data("amount"));
    ...
    //             // Calculate the new total (resetting to the original subtotal first)
    //             const newTotal = originalSubtotal + selectedShippingCharge;

    //             // Update the total display
    //             $("#total_amount")
    //                 .data("amount", newTotal.toFixed(2)) // Update the data-amount attribute
    //                 .html(`$${newTotal.toFixed(2)}`); // Update the displayed total
    //         });
    //     });
