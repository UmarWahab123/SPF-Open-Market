                ...
                <div class="billing_address">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 mb_10">
                            <h3 class="check_v3_title2">Choose your Shipping Method First <span style="color: red;">*</span></h3>
                                @php
                                $offerList = @$shippingCharges['response']['offerList'];
                                @endphp
                                    @foreach ($offerList as $offer)
                                        @if (isset($offer['totalOfferPrice']) && isset($offer['offeredProductList'][0]['shopRQShipment']['timeInTransit']))
                                            @php
                                                $offerPrice = $offer['totalOfferPrice'];
                                                $serviceDescription = $offer['offeredProductList'][0]['shopRQShipment']['timeInTransit']['serviceDescription'] ?? 'N/A';
                                            @endphp
                                            <div>
                                                <input 
                                                    type="radio" 
                                                    ...
                                                    class="shipping_method_radio"
                                                    id="method_{{ $loop->index }}" required
                                                >
                                                <label for="method_{{ $loop->index }}">${{ $offerPrice['value'] }} - {{ $serviceDescription }} </label>
                                            </div>
                                        @endif
                                    @endforeach
                        </div>
                        <div class="col-12 mb_10 mt-2">
                                            ...
                                            <span class="accordion-button shadow-none">
                                                <span class="w-100">
                                                    <label class="primary_checkbox d-inline-flex style4 gap_10 w-100" >
                                                        <input type="radio" name="payment_method" class="payment_method"  data-bs-toggle="collapse" data-bs-target="#collapse{{$key}}"  aria-controls="collapse{{$key}}" data-name="{{$payment->method}}" data-id="{{encrypt($payment->id)}}" value="{{$payment->id}}" {{$key == 0?'checked':''}}>
                                            ...
                                            <div class="accordion-body" id="acc_{{$payment->id}}">
                                                ...
                                                <div class="row">
                                                    ...
                                                    @elseif($payment->method == 'PayPal')
                                                        @include('frontend.amazy.partials.payments.payments_paypal')
                        ...
                        @endphp
                        <div class="col-12 mb_10 d-none @if($delivery_info && $delivery_info['delivery_type'] == 'pickup_location') d-none @endif">
                            <h3 class="check_v3_title2">{{__('common.billing_address')}}</h3>
                        </div>
                        <div class="col-12 d-none @if($delivery_info && $delivery_info['delivery_type'] == 'pickup_location') d-none @endif">
        ...
        <div class="order_sumery_box flex-fill">
            @if(!isModuleActive('MultiVendor'))
                ...
                @php
                    ...
                    if (app('general_setting')->price_with_vat){
                        $total = ($subtotal + @$newShippingCharge) -  $sub_discount;
                    }else{
                        $total = ($subtotal + $tax + @$newShippingCharge) - $sub_discount;
                    }
                @endphp
            @endif
            ...
            @if(isModuleActive('MultiVendor'))
                @php
                    $total = ($subtotal_without_discount + @$newShippingCharge) - @$sub_discount;
                @endphp
            @endif
            <div class="subtotal_lists">
                ...
                <div class="single_total_list d-flex align-items-center flex-wrap">
                    ...
                    <div class="single_total_right" id="shipping_charge">
                        <span>+ {{single_price(@$newShippingCharge)}}</span>
                    </div>
                </div>
                <div class="single_total_list d-flex align-items-center flex-wrap">
                    <div class="single_total_left flex-fill">
                    ...
                    <div class="single_total_right" id="subscribtion_discount">
                        <span>- {{single_price($sub_discount)}}</span>
                    </div>
                </div>
